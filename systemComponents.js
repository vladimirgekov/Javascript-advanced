function systemComponents(input) {
  ...
  let systemsSorted = Array.from(systems.keys()).sort((s1, s2) =>
    sortSystems(s1, s2)
  );

  for (let system of systemsSorted) {
    console.log(system);
    let componentsSorted = Array.from(
      systems.get(system).keys()
  ...
  function sortComponents(system, c1, c2) {
    return (
      systems.get(system).get(c2).length - systems.get(system).get(c1).length
    );
  }
}
systemComponents([
